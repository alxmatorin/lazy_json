...
public final class JsonBytesBenchmark {
    ...
    public static void main(String[] args) {
        ...
        measure("findAll/single", object, doc -> LazyJson.of(doc).findAll(JsonPath.compile("$id")).size());
        JsonPath id = JsonPath.compile("$id");
        measure("put/small", small, doc -> LazyJson.of(doc).set(id, value).length);
        measure("put/large", object, doc -> LazyJson.of(doc).set(id, value).length);
        List<Edit> single = List.of(Edit.raw("$id", "123"));
        measure("apply/single", small, doc -> LazyJson.of(doc).apply(single).length);
        List<Edit> batch = List.of(Edit.raw("$id", "123"), Edit.raw("$nested.id", "456"));
        measure("apply/batch", small, doc -> LazyJson.of(doc).apply(batch).length);
    ...
    static void measureFind(String name, byte[] doc, String path) {
        ...
        measure(name, doc, input -> LazyJson.of(input).find(compiled).length());
    }
