...
public final class RootAndEditsBenchmark {
    ...
    private static void measureFinds(List<byte[]> documents) throws Exception {
        ElementPath start = ElementPath.compile("$key3.key4");
        ElementPath tail = ElementPath.compile("$key1.key2");
        ElementPath hinted = ElementPath.compile("$<key1.key2");
        ElementPath root = ElementPath.compile("$key1");
        measure("find/cold-start", i -> JsonBytes.of(documents.get(i)).find(start).length());
        measure("find/cold-tail", i -> JsonBytes.of(documents.get(i)).find(tail).length());
        measure("find/cold-tail-hint", i -> JsonBytes.of(documents.get(i)).find(hinted).length());
        JsonBytes[] cached = indexed(documents);
        measure("find/cached-root", i -> cached[i].find(root).length());
        measure("find/cached-tail", i -> cached[i].find(tail).length());
        JsonBytes[] escaped = indexed(escapedDocuments(documents));
        measure("find/cached-escaped-root", i -> escaped[i].find(root).length());
    }

    private static JsonBytes[] indexed(List<byte[]> documents) {
        return documents.stream().map(doc -> {
            JsonBytes json = JsonBytes.of(doc);
            json.find("$absent");
            return json;
        }).toArray(JsonBytes[]::new);
    }
    ...
    private static void measureEdits(List<byte[]> documents) {
        ...
        for (int count : new int[]{2, 10}) {
            JsonBytes.Edits[] edits = Arrays.stream(cached).map(json -> batch(json, count)).toArray(JsonBytes.Edits[]::new);
        ...
        ElementPath replacement = ElementPath.compile("$<key1.key2");
        ElementPath insertion = ElementPath.compile("$<key1.payload");
        measure("large/replace", i -> cached[i].set(replacement, large).length);
        measure("large/insert", i -> cached[i].set(insertion, large).length);
    }

    private static JsonBytes.Edits batch(JsonBytes json, int count) {
        JsonBytes.Edits edits = json.edit().setRaw("$<key1.key2", "[7]");
