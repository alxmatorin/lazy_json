...
import ru.sber.jsonbytes.impl.CompiledEdits;
import ru.sber.jsonbytes.impl.PathTrie;
import ru.sber.jsonbytes.impl.RootIndex;
import ru.sber.jsonbytes.impl.Splicer;
import ru.sber.jsonbytes.impl.TrieWalker;
 ...
 * <pre>
 *   JsonBytes json = JsonBytes.of(bytes);
...
public final class JsonBytes {
    ...
    public static JsonBytes of(byte[] doc) {
        return new JsonBytes(doc, JsonEncoder.DEFAULT);
    }

    public static JsonBytes of(byte[] doc, JsonEncoder encoder) {
        return new JsonBytes(doc, encoder);
    }
    ...
    public Slice find(ElementPath path) {
    ...
    public List<Slice> findAll(List<String> keys) {
        return findAll(ElementPath.of(keys));
    }

    public List<Slice> findAll(String[] keys, int from) {
        return findAll(ElementPath.of(keys, from));
    }

    public List<Slice> findAll(ElementPath path) {
    ...
    public byte[] set(String path, Object value) {
        return set(ElementPath.compile(path), value);
    }

    public byte[] set(List<String> keys, Object value) {
        return set(ElementPath.of(keys), value);
    }

    public byte[] set(String[] keys, int from, Object value) {
        return set(ElementPath.of(keys, from), value);
    }

    public byte[] set(ElementPath path, Object value) {
    ...
    public byte[] setRaw(List<String> keys, String json) {
        return set(ElementPath.of(keys), json.getBytes(StandardCharsets.UTF_8));
    }
    ...
    public final class Edits {
        ...
        public Edits set(ElementPath path, Object value) {
