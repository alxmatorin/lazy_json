 ...
 * (после {@code }} — только пробелы). Ключи корня считаются уникальными: встретив все искомые, обход
 * останавливается, так что при дублях видно первое вхождение по направлению обхода.
 * <p>
...
public final class ElementPath {
    ...
    private static final ConcurrentHashMap<List<String>, ElementPath> KEYS_CACHE = new ConcurrentHashMap<>();
    ...
    public static ElementPath compile(String text) {
        ElementPath cached = CACHE.get(text);
        if (cached != null) {
            return cached;
        }
        boolean rootFromEnd = text.startsWith("$<");
        ElementPath path = new ElementPath(text, parse(text, rootFromEnd), rootFromEnd);
     ...
     */
    public static ElementPath of(List<String> keys) {
        ElementPath cached = KEYS_CACHE.get(keys);
        if (cached != null) {
            return cached;
        }
        ...
        ElementPath path = new ElementPath(textOf(names, rootFromEnd), names.stream().<Segment>map(Key::new).toList(), rootFromEnd);
    ...
    public static ElementPath of(String... keys) {
