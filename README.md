...
| `$["a'].b['c"]` | Буквальный ключ `a'].b['c`; внутри двойных кавычек используется JSON-экранирование |
...
Последовательность `']` завершает такой ключ. Для произвольных имён доступна форма `["имя"]` со стандартным JSON-экранированием: `\"`, `\\`, `\n`, `\uXXXX` и остальными JSON escape-последовательностями.
Например, `$["a'].b['c"]` выбирает один буквальный ключ `a'].b['c`, а `$["line\nbreak"]` — ключ с переводом строки.
Список ключей и `ElementPath.of(...)` по-прежнему принимают сами имена без экранирования. `ElementPath.toString()` автоматически использует двойные кавычки для имён с `']`; повторный `compile` сохраняет сегменты и направление обхода.
